...
- **Task 003: 공통 컴포넌트 라이브러리 구현** ✅ - 완료
  - ✅ shadcn/ui 추가 컴포넌트 설치 (Avatar, Dialog, Select, Sonner, Skeleton — Form은 Task 004에서 react-hook-form/zod와 함께 설치 예정)
  - ✅ 이벤트 카드 컴포넌트 구현
  - ✅ 참여자 프로필 카드 컴포넌트 구현
  - ✅ 로딩 스켈레톤 컴포넌트 구현
  - ✅ 빈 상태 UI 컴포넌트 구현
...
- **Task 004: 주최자 모바일 UI/UX 완성** ✅ - 완료 (인증 가드로 인한 실기 테스트 미완료 항목 있음, `docs/tasks/task-004.md` 참고)
  - ✅ 홈 페이지 (랜딩) UI 구현 (F001)
  ...
  - ✅ 주최자 전용 네비게이션 바 및 액션 버튼
    - ✅ "+ 이벤트 만들기" FAB 버튼 구현 (목록 페이지 자체 FAB로 구현)
  - ✅ 반응형 디자인 및 다크 모드 적용 (테마 토큰 기반, 코드 리뷰로 확인)
  - **주요 차이점**: 이벤트 생성, 수정, 삭제 권한 보유
  - ⚠️ **알려진 제약**: `proxy.ts` 인증 가드로 `/events` 등 보호된 라우트는 실 로그인 세션 없이 Playwright 실기 테스트를 하지 못함(홈페이지만 직접 검증). 타입체크/린트/코드 리뷰로 대체 검증했으며, 로그인 세션 확보 후(Task 008 이후) 재검증 필요

- **Task 005: 참여자 모바일 UI/UX 완성** ✅ - 완료 (인증 가드로 인한 실기 테스트 미완료 항목 있음, `docs/tasks/task-005.md` 참고)
  ...
  - **참여자 전용 네비게이션 바**: 라우트를 주최자 뷰와 공유하는 구조(같은 URL에서 역할별 조건부 렌더링)로 설계해, 전역 하단 내비게이션은 분리하지 않고 이벤트 상세 페이지 단위로 권한을 제어함 (`docs/tasks/task-005.md` 설계 방침 참고)
  - ✅ 반응형 디자인 및 다크 모드 적용 (테마 토큰 기반, 코드 리뷰로 확인)
  - **주요 차이점**: 읽기 전용 뷰, 초대 링크로만 참여 가능
  - **의존성**: Task 004 (공통 컴포넌트 재사용)

- **Task 006: 관리자 데스크톱 페이지 UI 완성** ✅ - 완료 (인증 가드로 인한 실기 테스트 미완료 항목 있음, `docs/tasks/task-006.md` 참고)
  - ✅ 관리자 로그인 페이지 UI 구현
  ...
  - ⚠️ **알려진 제약**: `proxy.ts` 인증 가드 및 관리자 role 체크 미구현(Task 008 예정)으로 `/admin/**` Playwright 실기 테스트 불가 — 타입체크/린트로 대체 검증. Task 008/011 완료 후 재검증 필요

### Phase 3: 데이터베이스 설정 및 핵심 기능 구현

- **Task 007: 데이터베이스 스키마 및 Supabase 초기 설정** ✅ - 완료 (`docs/tasks/task-007.md` 참고)
  ...
  - ✅ Realtime 구독 설정 준비 (events, event_participants publication 포함 확인)
...
- **Task 009: 이벤트 CRUD 및 초대 시스템** ✅ - 완료 (`docs/tasks/task-009.md` 참고)
  - ✅ 이벤트 생성 API 구현 (F001)
  - ✅ 초대 코드 자동 생성 로직 구현 (F002)
  - ✅ 이벤트 수정/삭제 API 구현 (F006)
  - ✅ 커버 이미지 업로드 기능 구현 (F009)
  - ✅ 초대 링크 공유 기능 구현 (클립보드만 구현, 카카오톡은 범위 제외) (F003)
  - ✅ 이벤트 상태 자동 관리 로직 구현 (read-time 파생 계산, DB 배치 없음) (F008)
  - ✅ Playwright MCP를 활용한 이벤트 생성/수정/삭제 테스트 (자동화 가능한 범위)
  - ⚠️ **알려진 제약**: Google OAuth 동의 화면을 Playwright로 자동화할 수 없어 실 로그인 세션이 필요한 생성/수정/삭제/업로드 플로우는 미검증(코드 리뷰 + 타입체크/린트로 대체 검증). `docs/tasks/task-009.md`의 수동 검증 권장 항목 참고

- **Task 010: 참여자 관리** ✅ - 완료 (`docs/tasks/task-010.md` 참고)
  - ✅ 초대 링크 참여 로직 구현 (F004)
  - ✅ 중복 참여 방지 로직 구현
  - ✅ 실시간 참여자 수 카운트 업데이트
  - ✅ 내가 참여한/만든 이벤트 목록 조회 구현 (F007)
  - ✅ Playwright MCP를 활용한 실시간 참여자 업데이트 테스트 (자동화 가능한 범위)
